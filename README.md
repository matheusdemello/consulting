...
- Project details use native HTML disclosures. The robotics case is text only; the other cases include inline SVG diagrams styled in `css/studies.css`. Radio controls select diagram layers and explanations through CSS, so they work without JavaScript. The illustrations use synthetic geometry, not client records, patient scans, or measured results. Project content is presented by problem and outcome, rather than employment history.
